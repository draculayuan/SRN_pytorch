...
def init_bias(module, cls_loss_type, init_prior, num_classes):
    ...
    if cls_loss_type == 'sigmoid_focal_loss':
        for m in module.modules():
            if isinstance(m, nn.Conv2d):
                torch.nn.init.constant_(m.bias.data, -math.log(1.0 / init_prior - 1.0))
...
class ResNetSRN(SRN):
    ...
    def _upsample_add(self, x, y):
        ...
        if self.interpolation == 'nearest':
            #return F.upsample(x, size=(H,W), mode='nearest') + y depreciated
            return F.interpolate(x, size=(H,W), mode='nearest') + y
        else:
            #return F.upsample(x, size=(H,W), mode='bilinear') + y depreciated
